...
const QuoteDetails: React.FC<QuoteDetailsProps> = ({
  ...
  return <ErrorBoundary fallback={<div className="p-2 bg-red-50 rounded text-sm">שגיאה בטעינת פרטי ההצעה</div>}>
      <div className="mt-2 space-y-2">
        <div className={`flex ${isMobile ? 'flex-col gap-1' : 'flex-row gap-6 space-x-reverse'}`}>
          ...
          {estimatedTime && <div className="flex items-center">
        ...
        {sampleImageUrl && <div className="mt-2">
            {/* תמונה ממוזערת + כפתור הגדלת תמונה */}
            <div className="relative inline-block">
              <img src={sampleImageUrl} alt="דוגמת עבודה" className="rounded-md max-h-16 object-cover cursor-zoom-in border border-gray-200 shadow hover:shadow-lg transition" onClick={() => setShowImageDialog(true)} title="להגדלה לחץ על התמונה" onError={e => {
            e.currentTarget.style.display = 'none';
          }} loading="lazy" style={{
            maxWidth: 120,
            minWidth: 80
          }} />
              <Button type="button" size="icon" className="absolute left-1 bottom-1 p-1 bg-white bg-opacity-80 hover:bg-opacity-100 shadow border border-gray-300" onClick={() => setShowImageDialog(true)} aria-label="הצג תמונה בגודל מלא">
            ...
            <Dialog open={showImageDialog} onOpenChange={setShowImageDialog}>
              <DialogContent className="max-w-3xl bg-white shadow-2xl z-50 rounded-xl flex flex-col items-center">
                ...
                <div className="w-full flex justify-center my-2">
                  <img src={sampleImageUrl} alt="דוגמת עבודה בגודל מלא" className="rounded-lg shadow max-h-[70vh] max-w-full border border-gray-200" loading="lazy" />
                </div>
                <DialogClose asChild>
