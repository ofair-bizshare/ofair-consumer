  ...
  if (isMobile) {
    return <div className="space-y-2">
        ...
        <Button variant="outline" className="w-full justify-start relative" onClick={handleNotificationsClick}>
          ...
          <span>התראות</span>
          {notificationsCount > 0 && <span className="absolute top-1 right-1 bg-red-500 text-white text-xs w-5 h-5 flex items-center justify-center rounded-full">
              {notificationsCount}
            </span>}
        </Button>
  ...
  return <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <Button variant="ghost" className="hidden md:flex items-center space-x-2 text-blue-700 hover:text-blue-800 hover:bg-blue-50 relative">
          <UserCircle size={18} />
          <span>פרופיל</span>
          {notificationsCount > 0 && <span className="absolute top-0 right-0 bg-red-500 text-white text-xs w-5 h-5 flex items-center justify-center rounded-full">
              {notificationsCount}
            </span>}
        </Button>
      </DropdownMenuTrigger>
      <DropdownMenuContent align="end" className="w-56">
        ...
        <DropdownMenuItem onClick={handleNotificationsClick} className="relative">
          <Bell className="mr-2 h-4 w-4" />
          <span>התראות</span>
          {notificationsCount > 0 && <span className="ml-auto bg-red-500 text-white text-xs w-5 h-5 flex items-center justify-center rounded-full">
              {notificationsCount}
            </span>}
        </DropdownMenuItem>
