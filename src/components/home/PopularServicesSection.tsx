...
const popularProfessions = professions.filter(p => ['renovations', 'electricity', 'plumbing', 'carpentry', 'interior_design', 'painting'].includes(p.id));
...
const popularServices: ServiceItem[] = [{
  ...
  color: 'bg-red-500',
  serviceId: 'electricity',
  displayName: 'חשמל ותאורה'
}, {
  icon: <Home size={32} />,
  name: 'עיצוב פנים',
  color: 'bg-indigo-500',
  serviceId: 'interior_design',
  displayName: 'עיצוב פנים'
}];
  ...
  return <section className="py-14 bg-white">
      <div className="container mx-auto px-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-6 stagger-animation">
          {popularServices.map((service, idx) => <Link key={idx} to={`/search?profession=${service.serviceId}&location=all&displayName=${encodeURIComponent(service.displayName)}`} className="group">
              <div className="flex flex-col items-center justify-center p-4 rounded-xl bg-white border border-gray-100 hover:shadow-lg transition-all duration-300 hover:-translate-y-1">
                ...
                <h3 className="text-lg font-medium text-gray-800 text-center">{service.name}</h3>
              </div>
            </Link>)}
        </div>
      </div>
